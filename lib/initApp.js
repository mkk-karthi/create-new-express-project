...
const initApp = (language, answers, __dirname) => {
  ...
  const createProjectStructure = () => {
    ...
    fs.mkdirSync(`${projectName}/src/logs`);

    if (database == "mysql_sequelize") fs.mkdirSync(`${projectName}/src/seeders`);
    if (tools.includes("jest")) fs.mkdirSync(`${projectName}/src/tests`);
  };
  ...
  const createEnv = () => {
    ...
    if (tools.includes("jest")) fs.writeFileSync(`${projectName}/.env.test`, envContent.join("\n"));
  };
  ...
  const createProject = () => {
    ...
    fs.cpSync(
      ...
      path.join(projectName, "src", "utils/logger.js")
    );
    ...
    if (tools.includes("swagger")) {
      fs.cpSync(
        ...
        path.join(projectName, "src", "utils/swagger.js")
      );
    }
    if (tools.includes("jest")) {
      fs.cpSync(
        path.join(__dirname, "dist", "tests/user.test.js"),
        path.join(projectName, "src", "tests/user.test.js")
      );
      fs.cpSync(
        path.join(__dirname, "dist", "utils/jest.config.js"),
        path.join(projectName, "jest.config.js")
      );

      if (database == "mysql_sequelize") {
        fs.cpSync(
          path.join(__dirname, "dist", "utils/jest.setup.js"),
          path.join(projectName, "jest.setup.js")
        );
      }
    }
  };
