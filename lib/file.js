...
const initApp = (language, answers, __dirname) => {
  ...
  const createProjectStructure = () => {
    ...
    if (language == "js") {
      ...
      fs.mkdirSync(`${projectName}/middlewares`);
      fs.mkdirSync(`${projectName}/services`);
      fs.mkdirSync(`${projectName}/storage`);
  ...
  const createPackage = async () => {
    ...
    let packageContent = {
      ...
      version: "1.0.0",
      description: "",
      main: language == "js" ? "app.js" : "src/app.ts",
      type: "module",
      scripts,
      keywords: [],
      author: "",
  ...
  const createProject = async () => {
    ...
    // move dist files
    fs.cpSync(path.join(__dirname, "dist", "controllers"), path.join(projectName, "controllers"), {
      recursive: true,
    });
    if (viewEngine) {
  ...
  const createEnv = () => {
    fs.writeFileSync(`${projectName}/.env`, `PORT=8000`);
  };

  const createConfig = () => {
    fs.writeFileSync(
      `${projectName}/config/app.js`,
      `export default ${JSON.stringify(
        {
          ...
          whiteListOrigins: ["localhost:8080", "localhost:8000"],
        },
        null,
        2
      )}`
  ...
  const createRoute = () => {
    ...
    if (language == "js") {
      fs.writeFileSync(`${projectName}/routes/index.js`, routeContent.join("\n"));
    } else {
  ...
  const createApp = () => {
    ...
    if (language == "js") {
      ...
      appContent.push(`import rateLimit from "express-rate-limit";`);
      appContent.push(
      ...
      fs.writeFileSync(`${projectName}/app.js`, appContent.join("\n"));
    } else {
